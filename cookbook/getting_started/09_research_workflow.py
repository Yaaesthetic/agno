...
#         self, topic: str, use_search_cache: bool, num_attempts: int = 1
...
# # Simplified imports for clarity
# import json
# from textwrap import dedent
# from agno.agent import Agent
# from agno.models.openai import OpenAIChat
...
# # Keep the Pydantic models as they define the data structure
# from pydantic import BaseModel, Field
...
# class SimpleResearchWorkflow(Workflow):
#     debug_mode=True
#     ## 1. Define the team of specialist agents
#     web_searcher: Agent = Agent(
#         model=OpenAIChat(id="gpt-4o-mini"),
#         tools=[DuckDuckGoTools()],
#         response_model=SearchResults,
#         instructions="You are a research assistant. Find 5 relevant articles for the given topic.",
#         debug_mode=True,
...
#         tools=[Newspaper4kTools()],
#         response_model=ScrapedArticle,
#         instructions="You are a content extractor. Scrape the content from the given URL.",
#         debug_mode=True,
#     )
#     writer: Agent = Agent(
#         model=OpenAIChat(id="gpt-4o-mini"),
#         instructions="You are an academic writer. Synthesize the provided content into a short summary report.",
#         markdown=True,
#     )

#     ## 2. The "Plan" is the .run() method
#     def run(self, topic: str):
#         """This is the core logic: Search -> Scrape -> Write."""
#         print(f"🚀 Starting workflow for topic: {topic}")

#         # STEP 1: Call the 'web_searcher' agent
#         print("   -> Step 1: Searching for articles...")
#         searcher_response = self.web_searcher.run(topic)
#         articles_to_scrape = searcher_response.content.articles

#         # STEP 2: Call the 'article_scraper' agent for each article
#         print(f"   -> Step 2: Scraping {len(articles_to_scrape)} articles...")
#         scraped_content = []
#         for article in articles_to_scrape:
#             scraper_response = self.article_scraper.run(article.url)
#             if scraper_response.content:
#                 scraped_content.append(scraper_response.content.content)

#         # STEP 3: Call the 'writer' agent with the combined content
#         print("   -> Step 3: Writing the final report...")
#         final_input = f"Topic: {topic}\n\n" + "\n\n---\n\n".join(scraped_content)
        
#         # Yield the final streamed response
#         yield from self.writer.run(final_input, stream=True)
#         print("\n✅ Workflow complete!")

# # -------------------------------------------------------------------
# # HOW TO RUN THE SIMPLIFIED WORKFLOW
# # -------------------------------------------------------------------

# if __name__ == "__main__":
#     # Create an instance of our simple workflow
#     research_workflow = SimpleResearchWorkflow()

#     # Define the topic to research
#     my_topic = "latest breakthroughs in fusion energy"

#     # Run the workflow and get the streaming response
#     report_stream = research_workflow.run(topic=my_topic)

#     # Print the final report as it comes in
#     pprint_run_response(report_stream, markdown=True)


# Imports for the workflow, now including storage
import json
from textwrap import dedent
from agno.agent import Agent
from agno.models.openai import OpenAIChat
from agno.storage.sqlite import SqliteStorage  # <-- Added import
from agno.tools.duckduckgo import DuckDuckGoTools
from agno.tools.newspaper4k import Newspaper4kTools
...
# -------------------------------------------------------------------
# THE SIMPLIFIED WORKFLOW WITH CACHING
# -------------------------------------------------------------------

class SimpleResearchWorkflow(Workflow):
    debug_mode=True
    # The team of agents remains exactly the same
    web_searcher: Agent = Agent(
        model=OpenAIChat(id="gpt-4o-mini"),
        tools=[DuckDuckGoTools(fixed_max_results=2)],
        response_model=SearchResults,
        instructions="You are a research assistant. Find 2 or one no more relevant articles for the given topic.",
    ...
    # The .run() method is now updated to check the cache at each step
    def run(self, topic: str):
        """The workflow plan now includes checking for and saving cached data."""
        print(f"🚀 Starting workflow for topic: {topic}")

        # STEP 1: Search for articles (with caching)
        if "search_results" in self.session_state:
            print("   -> Step 1: Found search results in cache. Loading...")
            ...
            if not isinstance(searcher_response.content, SearchResults):
                yield WorkflowCompletedEvent(content="Failed to get search results.")
                return
        ...
        articles_to_scrape = search_results.articles

        # STEP 2: Scrape articles (with caching)
        if "scraped_content" in self.session_state:
            print(f"   -> Step 2: Found {len(self.session_state['scraped_content'])} scraped articles in cache. Loading...")
            scraped_content = self.session_state["scraped_content"]
        else:
            print(f"   -> Step 2: Scraping {len(articles_to_scrape)} articles...")
            scraped_content = []
            for article in articles_to_scrape:
                ...
                if scraper_response.content:
                    scraped_content.append(scraper_response.content.content)
            # Save to cache
            self.session_state["scraped_content"] = scraped_content
            print("      -> Saved scraped content to cache.")

        # STEP 3: Write the final report (no caching for the final output in this simple version)
        print("   -> Step 3: Writing the final report...")
        final_input = f"Topic: {topic}\n\n" + "\n\n---\n\n".join(scraped_content)
        
        yield from self.writer.run(final_input, stream=True)
        print("\n💾 Saving full workflow state to cache...")
        # self.write_to_storage()
        print("\n✅ Workflow complete!")

# -------------------------------------------------------------------
# HOW TO RUN THE WORKFLOW WITH STORAGE
# -------------------------------------------------------------------

if __name__ == "__main__":
    my_topic = "brief breakthroughs in fusion energy"
    
    # 1. Create a unique session ID based on the topic
    # This ensures that a search for a different topic gets its own cache.
    url_safe_topic = my_topic.lower().replace(" ", "-")
    session_id = f"simple-research-on-{url_safe_topic}"

    # 2. Initialize the storage object
    # This will create a file at 'tmp/simple_workflows.db'
    storage = SqliteStorage(
        table_name="simple_research_workflow",
        db_file="tmp/simple_workflows.db",
    )

    # 3. Create an instance of the workflow, now passing storage and session_id
    research_workflow = SimpleResearchWorkflow(
        storage=storage,
    ...
    try:
        print("\n💾 Saving full workflow state to cache...")
        research_workflow.write_to_storage()
